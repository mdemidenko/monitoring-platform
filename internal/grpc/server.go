...
// Принимает domain.NotificationService вместо *core.NotificationService
func NewGRPCServer(
    cfg *config.Config,
    notificationService domain.NotificationService, // ✅ Интерфейс, а не конкретная структура
) (*GRPCServer, error) {

    // Создаем реализацию сервиса с интерфейсом
